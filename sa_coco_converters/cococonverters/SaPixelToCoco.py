...
import numpy as np
def sa_pixel_to_coco_instance_segmentation( make_annotation, image_commons, id_generator):
    ...
    for instance in sa_ann_json:
        ...
        output_img , contours, hierarchy, *_ = cv.findContours(databytes, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_NONE)
        ...
        segmentation =[contour.flatten().tolist() for contour in contours if len(contour.flatten().tolist()) >= 5]

        annotations_per_image.append(make_annotation (
