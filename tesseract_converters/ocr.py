...
class ocr:
    ...
    def load_data(self):
        ...
        os.makedirs(self.output_dir +'/texts', exist_ok=True)
        os.makedirs(self.output_dir +'/images', exist_ok=True)
        os.makedirs(self.output_dir +'/jsons', exist_ok=True)
    ...
    def predict(self, output_type = 'txt'):
        for im_in, im_out in zip(self.input_image_list, self.output_image_list):
            if output_type == 'txt':
                output_path = self.output_dir + '/texts/' + im_out + 'txt'
                tf = open(output_path, "wt")
                result = image_to_string(im_in + "tif", config='--oem 1')
                tf.write(result)
                tf.close()
            elif output_type == 'json':
                output_path = self.output_dir + '/jsons/' + im_out + 'json'
        ...
        for im_in, im_out in zip(self.input_image_list,
            ...
            for i in range(len(file_json["level"])):
                if file_json["text"][i] != "":
                    ...
                    draw.text((x1, y1), file_json["text"][i], fill='red', font=font)
                    draw.rectangle(((x1, y1), (x2, y2)), outline='red')

            axis("off")

            output_path = self.output_dir + '/images/' + im_out + 'jpg'
