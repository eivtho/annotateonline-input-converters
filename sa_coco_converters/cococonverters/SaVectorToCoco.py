...
def sa_vector_to_coco_instance_segmentation(make_annotation, image_commons, id_generator):
    ...
    for polygon_group in grouped_polygons.values():
        ...
        for cat_id, polygons in polygon_group.items():
            try:
                masks = cocomask.frPyObjects(polygons,image_info['height'], image_info['width'] )
...
def sa_vector_to_coco_keypoint_detection(json_paths, id_generator, id_generator_anno,id_generator_img, make_image_info):

    def __make_skeleton(template):
        res =  [[connection['from'], connection['to']] for connection in template['connections']]
        return res

    def __make_keypoints(template):
        int_dict = {int(key): value for key, value in template['pointLabels'].items()}
    ...
    def __make_bbox(points):
        ...
        return [int(min(xs)), int(min(ys)),int(max(xs)) - int(min(xs)), int(max(ys)) - int(min(ys))]
    ...
    for path_ in json_paths:
        ...
        for instance in json_data:
            ...
            if instance['type'] != 'template' or instance['templateName'] in template_names:
                continue
        ...
        for instance in json_data:
            ...
            if instance['type'] == 'template':
                ...
                annotation = __make_annotations(instance, id_generator_anno, cat_id, image_info['id'])
